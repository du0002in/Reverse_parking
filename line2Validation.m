function pass=line2Validation(Ctmp, Etmp, Fptmp, theta_pre)


Combtmp=npermutek(1:4,2);
Counter=0;
...
for i=1:length(Comb)
    ...
    if abs(T(Comb(i,1))-T(Comb(i,2)))<10*pi/180
        if (Comb(i,1)==2 && Comb(i,2)==4) || (Comb(i,1)==4 && Comb(i,2)==2)
            if abs(d(2)-d(4))>1 && abs(d(2)-d(4))<2
                pass=1;
                return;
            end
        elseif (Comb(i,1)==1 && Comb(i,2)==3) || (Comb(i,1)==3 && Comb(i,2)==1)
