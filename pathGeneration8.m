function [status, waypoints, trajectory, S, manuver]=pathGeneration8(theta,x,z,buffer,varargin)
...
if abs(theta)<=0.01 && z<L_slot-0.35+buffer
    ...
    if isempty(find(x_car_global>d(2)-side_torlerence,1)) && isempty(find(x_car_global<d(4)+side_torlerence,1))
        status=1;
        manuver(1:3,:)=4;
        SP=[0 0]; EP=[x_car_global(3) L_slot-z];
        B=[0 0];C=[0 0];D=[0 0];
        ...
        if isempty(find(waypoints(:,2)<min_z,1))
            ...
            S(1)=distance(SP,EP);
            pathno=pathno+1;
            pathGen{pathno,:}={{drawpath},{waypoints},{trajectory},{S},{manuver}};
        else
            status=0;
        end
    else
...
if theta<=0
    ...
    if isinf(a)
        ...
        if N<sqrt((Rmin-0.5*W_car)^2-(0.5*(W_slot+0.1)-side_torlerence-Rmin)^2)+buffer
            ...
            xdumy=linspace(SP(1)+x, C(1)+x, ceil(abs(C(1)-SP(1))/0.2));
            zdumy=linspace(SP(2)+z,C(2)+z,length(xdumy));
...
% if isreal(min_tc) && isreal(min_ts)
if status==1
    if isreal(min_tc) && isreal(min_ts)
        if min_tc<0
            manuver(1,:)=[0 3];
...
if pathno==0
    ...
    for i=1:pathno
        pathdist(i)=sum(pathGen{i,1}{1,4}{1,1});
    end
    [~,indmin]=min(pathdist);
    status=1;
    drawpath=pathGen{indmin,1}{1,1}{1,1};
    waypoints=pathGen{indmin,1}{1,2}{1,1};
    trajectory=pathGen{indmin,1}{1,3}{1,1};
    S=pathGen{indmin,1}{1,4}{1,1};
    manuver=pathGen{indmin,1}{1,5}{1,1};
